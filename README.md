...
- 🔔 **Background Notifications** — A persistent daemon checks your watchlist every 60 seconds and sends a `critical`-priority desktop notification at the **30-minute**, **1-hour**, and **24-hour** marks before an episode airs.
...
ani-rem start
    ...
    │
    └─► YES ─► Enter the worker loop (checks every 60 seconds)
```
...
The worker calls `checkAiringAnime()` every 60 seconds. For each `"Currently Airing"` show it checks the countdown string returned by `GetTimeUntilAiring()` and fires a notification at these exact windows:

| Countdown string contains | Notification title |
|---|---|
| `"0h 30m"` | ⚡ **30 Mins Left!** |
| `"1h 0m"` | 🕐 **1 Hour Left!** |
| `"24h 0m"` | 📅 **Airing Tomorrow** |

The separate `check` command works slightly differently — it fires a notification any time the countdown contains `"Next episode in 0h"`, covering any show airing within the next 60 minutes.
...
The `-u critical` flag makes notifications **persistent** — they stay on screen until dismissed. The icon used is `-i input-tablet`.
...
Spawns a detached background worker that checks your watchlist every 60 seconds and fires desktop notifications at the 30-minute, 1-hour, and 24-hour milestones. Safe to run from a terminal, `.desktop` file, or startup script.
...
Manually triggers a single check of your watchlist. Prints each currently-airing show's countdown to the terminal and sends a notification for any show airing within the next 60 minutes.
...
│   └── notify.go         # notify-send wrapper with DISPLAY + DBUS injection
└── models/
